...
const PTOCalendar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHolidays = async () => {
      try {
        ...
        const countries = Array.from(new Set(Object.values(ptoData).map(data => data.nationality)))
  ...
  const employeesOutOfOffice = useMemo(() => {
    ...
    daysToShow.forEach(day => {
      ...
      const count = Object.values(ptoData).filter(data => 
        data.ptoDates.includes(dateStr) || data.sickDates.includes(dateStr)
      ).length
  ...
  const filteredRows = useMemo(() => {
    const allRows = Object.entries(ptoData).map(
      ([name, data], index) => {
        const row: RowData = {
          id: index + 1,
          employee: name,
        ...
        daysToShow.forEach((day) => {
          ...
          row[dateStr] = {
            ...
            hasSick,
            hasHoliday,
            show: (hasPTO && activeFilters.includes('vacation')) ||
      ...
      ...daysToShow.map((day) => {
        ...
        return {
          ...
          renderCell: (params: GridRenderCellParams) => {
            ...
            const cellData = params.value as { hasPTO: boolean; hasSick: boolean; hasHoliday: boolean; show: boolean }
            if (!cellData.show) return null

            const ptoPeriods = findContinuousPeriods(
              ptoData[params.row.employee].ptoDates || []
            )
            const sickPeriods = findContinuousPeriods(
              ptoData[params.row.employee].sickDates || []
            )
            const currentPTOPeriod = ptoPeriods.find((period) =>
              period.includes(params.field)
            )
            const currentSickPeriod = sickPeriods.find((period) =>
              period.includes(params.field)
            )
                ...
                {showHoliday && (
                  <Tooltip title={holidayName}>
                    <Box
                      ...
                      sx={{ width: 16, height: 16, opacity: 0.9, borderRadius: '50%', border: '2px solid #D2DEFF' }}
                    />
                  </Tooltip>
                )}
              </Box>
